...
- **Lifting state** - if sibling components need access to the same state, that
  state should be "lifted" into their **least common parent** and then passed
  into the components that need it
- **Colocating state** - keeping state closest to where it's needed. If state is
  only needed by one component, it should be stored in that component, not in
  some parent
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=03%3A%20Lifting%20state&em=yost90%40gmail.com).
