...
- In order to get a reference to a DOM node, we can set up a reference using
  `React.useRef()`, then set a React node's `ref` prop to that reference. Once
  that's setup, calling `<ref>.current` will give us the DOM node, if we make
  that call in a `React.useEffect()` method, since once that is called, the
  elements are rendered in the DOM
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=05%3A%20useRef%20and%20useEffect%3A%20DOM%20interaction&em=yost90%40gmail.com).
