...
- Side effect callback functions are called every time a component is rendered -
  after it is rendered
- **Lazy state initialization** - when using the `React.useState` hook, instead
  of passing an initial value (if getting that initial value is expensive), we
  can pass a function that gets the initial value. Then when a component is
  rendered, React won't call that intialization function every time, only when
  the state is missing a value (likely on the first render)
- **Effect dependencies** - the `React.useEffect` hook can take an optional
  second parameter, a dependency array. The dependency array can be of any
  length. The callback function will only be called if one or more of the
  dependencies have changed. React uses a shallow equality check here, so
  including an object in this dependency array will cause the callback function
  to be called everytime the component re-renders
- **Custom hooks** - functions that use React or other custom hooks. Custom
  hooks can be used to encapsulate logic that could be reused across other
  components. To standardize these their names should start with `use`
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=02%3A%20useEffect%3A%20persistent%20state&em=yost90%40gmail.com).
