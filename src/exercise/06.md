...
- When we need to make HTTP requests that will effect what's rendered on the
  webpage, we need to make them in a `React.useEffect()`
- We cannot use an async function in `React.useEffect()` so instead of awaiting
  the response, a simple way of handling this is to abstract all of the async
  logic to a separate function. Then inside `React.useEffect()` we can call that
  function and chain a `.then()` call to handle the result
- We can use `ErrorBoundarys` to handle errors. These are React nodes
  themselves. The easiest way to use them is to use the existing
  `react-error-boundary` package. The `ErrorBoundary` in this package accepts
  lots of useful props like `FallbackComponent`, `onReset`, and `resetKeys`
  - The `FallbackComponent` indicates what component to render in case of an
    error. The error and the `resetErrorBoundary` function are passed to this
    component
  - The `onReset` prop is a function that is called when the `ErrorBoundary` is
    reset. It should be used for resetting state
  - The `resetKeys` prop is an array containing a set of dependencies. If these
    dependencies are changed, the `ErrorBoundary` will automatically be reset
...
Fill out
[the feedback form](https://ws.kcd.im/?ws=React%20Hooks%20%F0%9F%8E%A3&e=06%3A%20useEffect%3A%20HTTP%20requests&em=yost90%40gmail.com).
